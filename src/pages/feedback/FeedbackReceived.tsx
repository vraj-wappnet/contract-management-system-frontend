...
import { Card, Table, Tag, Avatar, Rate, Typography, Tooltip, Statistic } from 'antd';
    ...
    },
    {
      ...
      render: (rating: number) => (
        <div className="flex items-center">
          <Rate disabled allowHalf value={rating} className="text-sm text-yellow-400" />
          <span className="ml-2 text-sm font-medium text-gray-700">{rating.toFixed(1)}/5</span>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (date: string) => (
        <div className="text-sm">
          <div className="font-medium text-gray-800">{dayjs(date).format('MMM D, YYYY')}</div>
        ...
        <Card className="shadow-lg rounded-xl bg-white p-4 animate-fade-in">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            ...
            <Statistic
              ...
              value={tableData.filter(item => item.status.toLowerCase() === 'completed').length}
              className="text-center"
            />
          </div>
        </Card>
            ...
            locale={{
              emptyText: (
                <div className="py-8 text-center">
                  <Text type="secondary" className="text-gray-500">No feedback data available</Text>
                </div>
              )
            }}
          />
